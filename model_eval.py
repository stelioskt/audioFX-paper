...
def get_embed(audio, processor, model, model_type, target_sr):
    ...
    if model_type == "mert":
        inputs = processor(tensor_audio, sampling_rate=target_sr, return_tensors="pt", padding=True)
        ...
        inputs = processor(audios=audio, sampling_rate=target_sr, return_tensors="pt", padding=True)
        inputs = {k: v.to(DEVICE) for k, v in inputs.items()}
        with torch.no_grad():
        ...
        inputs = processor(audios=audio, sampling_rate=target_sr, return_tensors="pt", padding=True, text=processor.audio_token)
        inputs = {k: v.to(DEVICE) for k, v in inputs.items()}
        with torch.no_grad():
...
def run_regression_pipeline(df, model_tag):
    ...
    for target_idx, name in enumerate(["Valence", "Arousal"]):
        ...
        print(f"[{model_tag}][{name}] MAE: {mae:.4f}, RMSE: {rmse:.4f}, R2: {r2:.4f}")
...
def run_classification_pipeline(df, model_tag):
    ...
    print(f"[{model_tag}][Classification] Accuracy: {acc:.4f}")
